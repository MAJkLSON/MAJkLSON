    ...
    public static void main(String[] args) {
        try (BufferedReader buffer = new BufferedReader(new FileReader("input.txt"))) {
            ...
            while ((line = buffer.readLine()) != null) {
                ...
                while (position < firstCompartment.length()) {
                    if (secondCompartment.contains(firstCompartment.substring(position, position+1))) {
                        break;
                    } else {
                        position += 1;
                    }
                }
                alphabeticResult += (firstCompartment.charAt(position));
            }

            position = 0;
            while (position < alphabeticResult.length()) {
                result += abc.indexOf(alphabeticResult.charAt(position)) + 1;
                position++;
            }
